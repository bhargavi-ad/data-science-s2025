...
- Is this sample representative of *all* houses in Helvig? Why or why
  not?
  - No this sample is not representative of all houses in Helvig because
    this sample was just the first 25 houses in the Helvig not a random
    sample of the houses in Helvig. Theoretically, people could have
    chosen to live near each other based on being related, distantly
    with the same last name or closely related. There could be other
    factors in terms of where these houses were located, obviously much
    closer together compared to a random 25 house sample, that would
    change how accurately the sample represents all of the houses in
    Helvig.
...
- Is there any reason that people with the same last name might tend to
  *live near each other*?
  - They could be extended family that want to live close together.
    Also, often times people live close to people with the same
    nationality as themselves and there are many repeated last names.
...
```
    ...
    ## 1 prevalence 0.0169    0.0861  0.169   0.05 percentile
...
- Note that we used the *random* sample with the bootstrap procedure in
  ...
  - Yes we could make the bootstrap to make a confidence interval of the
    sequential sample, but it would not be representative of all of
    Helvig.
  - This would not be representative of all of Helvig because the
    sequential sample would take a selective sample of Helvig that is
    not distributed throughout the population. Therefore, the confidence
    interval would be less likely to be representative of Helvig.
...
- Experimental because we are going to record their reaction time before
  and then provide them with substances and record their reaction time
  afterwards

#### Question / Hypothesis

- Is there a trend between body weight and reaction time when a person
  is given X amount of alcohol (or cannabis if possible)
...
- (What steps will you take to collect the data?)
  ...
  - Within each house we are going to only select individuals above the
    age of 21, for ethical considerations
    - We are aware that this would add some bias into the collection but
  ...
  - Record the individual’s body weight at the time of testing
    ...
    - Write a protocol. Document step-by-step instructions that your
      team will follow. That way, you can feasibly split up data
      collection among the whole team, while making sure each team
      member doesn’t make ad hoc decisions that introduce bias into your
      results.
...
- (For experiments only: How will you ensure any effects you observe are
  ...
  - We will collect all of our data in one sitting so that there are no
    other interacts the individual could have
  - Luckily this test does not involve waiting time
- Our sample size will be 30 people so that each person in our group
  only has to test 5 people.
